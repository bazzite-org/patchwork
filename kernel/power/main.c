 ...
 * 
 * When the system suspends, it will first enter the state "lps0", suspend,
 * and then restore the last state before entering "lps0". I.e., if userspace 
 * is not S0ix-aware, the transitions expected by Modern Standby devices will 
 * always be performed. Userspace cannot see or enter "lps0", as that is
 * the state where the kernel is suspended.
 */
...
static ssize_t standby_store(struct kobject *kobj, struct kobj_attribute *attr,
			   const char *buf, size_t n)
{
	standby_state_t state = decode_standby_state(buf, n);
	if (state >= PM_STANDBY_MAX)
		return -EINVAL;

	int error = pm_standby_transition(state);
	return error ? error : n;
}
