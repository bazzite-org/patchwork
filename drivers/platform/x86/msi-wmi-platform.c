...
struct msi_wmi_platform_data {
	...
	struct mutex cpu_fan_table_lock;	/* Protects RMW changes to the CPU fan table */
	struct mutex gpu_fan_table_lock;	/* Protects RMW changes to the GPU fan table */
	struct mutex fan_mode_lock;		/* Protects RMW changes to the fan mode flags */
	struct mutex fw_attr_lock;		/* Protects RMW changes to firmware attrs */
};
						...
						const char *buf, size_t count)
{
	...
	guard(mutex)(&data->cpu_fan_table_lock);
				  ...
				  int channel, long val)
{
	...
	case hwmon_pwm:
		switch (attr) {
		case hwmon_pwm_enable:
			guard(mutex)(&data->fan_mode_lock);
...
static int msi_wmi_fw_attrs_init(struct msi_wmi_platform_data *data)
{
	...
	err = devm_mutex_init(&data->wdev->dev, &data->fw_attr_lock);
	if (err)
		return err;
...
static int msi_wmi_platform_probe(struct wmi_device *wdev, const void *context)
{
	...
	ret = devm_mutex_init(&wdev->dev, &data->cpu_fan_table_lock);
	if (ret < 0)
		return ret;

	ret = devm_mutex_init(&wdev->dev, &data->gpu_fan_table_lock);
	if (ret < 0)
		return ret;

	ret = devm_mutex_init(&wdev->dev, &data->fan_mode_lock);
	if (ret < 0)
		return ret;
