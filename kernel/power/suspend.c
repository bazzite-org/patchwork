	...
	if (standby_current < state) {
		for (; standby_current < state; standby_current++) {
			...
			case PM_STANDBY_SLEEP:
				...
				break;
			case PM_STANDBY_LPS0:
				error = platform_standby_lps0_entry();
				break;
			}
		...
		for (; standby_current > state; standby_current--) {
			...
			case PM_STANDBY_LPS0:
				error = platform_standby_lps0_exit();
				break;
			case PM_STANDBY_SLEEP:
...
int pm_standby_transition(standby_state_t state)
{
	...
	int error = platform_standby_transition_internal(state);
	unlock_system_sleep(sleep_flags);
	return error;
}
...
static int enter_state(suspend_state_t state)
{
	...
	platform_standby_transition_internal(PM_STANDBY_LPS0);
