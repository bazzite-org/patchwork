...
time PATH="/usr/lib/ccache/bin:$PATH" \
    ...
    CCACHE_MAXSIZE=4G CCACHE_IGNORECONFIG=1 \
    make -s -j $(expr $(nproc) - 2)

# Sync to out dir
echo "Installing to ./out"
rm -rf out
mkdir -p out
make -j $(expr $(nproc) - 2) install INSTALL_PATH=./out/ -s
# sudo sbsign --key key.pem --cert key.cert.pem ./arch/x86/boot/bzImage --output vmlinuz
make -j $(expr $(nproc) - 2) modules_install INSTALL_MOD_PATH=./out/ -s
